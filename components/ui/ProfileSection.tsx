...
const ProfileSection = (props: { title: String; subtitle: String }) => {
  ...
  const colors =
    selectedOption == "home"
      ? "bg-light-blue border-light-blue dark:bg-dark-blue dark:border-dark-blue"
      : "bg-light-item-bg border-light-item-border dark:bg-dark-item-bg dark:border-dark-item-border";
  const titleTextColor =
    selectedOption == "home"
      ? "text-light-primary dark:text-dark-white"
  ...
  return (
    <Link href={"/"} onClick={profileClicked}>
      <div
        id=""
        className={
          "border-[1px] rounded flex mb-4 content-center py-1 px-3 w-full h-16 group " +
          colors
        }
      >
        <div className="grid grid-cols-[48px_auto] gap-2 md:gap-3 content-center w-full">
          ...
          <div className="place-self-start self-center">
            <p
              className={
                "capitalize text-xs " + titleTextColor + " font-semibold"
              }
            >
              {title}
            </p>
